...
class myENV:
    def __init__(self, config):
        self.STATENUM = config.screen_width
        self.MAXSTAE = config.max_state
        self.screen_1_hot = config.screen_1_hot
        self.pos = 0
        self.stp = 0
        self.screen = np.zeros(shape=[self.STATENUM, 1])

        if self.screen_1_hot:
    ...
    def step(self, action):
        ...
        if action == 0:
            self.pos += 1
            self.pos = min(self.pos, self.MAXSTAE - 1)
        elif action == 1:
            self.pos -= 1
            self.pos = max(self.pos, 0)

        reward = 0
        if self.pos == self.MAXSTAE - 1:
            reward = 1

        terminal = 0
        if self.stp == self.MAXSTAE + 9:
            terminal = 1
